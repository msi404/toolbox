...
const Switch = ({ children }) => {
    const validCase = React.Children.toArray(children).find(
    // @ts-ignore
    (child) => React.isValidElement(child) && child.props.when);
...
const Dynamic = ({ component: Component, ...props }) => {
    if (!Component)
        return null;
    return React.isValidElement(Component) ? (React.cloneElement(Component, props)) : (
    // @ts-expect-error
    jsxRuntime.jsx(Component, { ...props }));
};
