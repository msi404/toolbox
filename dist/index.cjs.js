...
const Show = ({ children, when, fallback }) => {
    ...
    return (jsxRuntime.jsxs(require$$0.Fragment, { children: [unwrappedWhen && children, !unwrappedWhen && fallback] }));
};

const Match = ({ when, children }) => {
    const unwrappedWhen = unwrap(when);
    return unwrappedWhen ? jsxRuntime.jsx(jsxRuntime.Fragment, { children: children }) : null;
};
const Switch = ({ children }) => {
    const validCase = require$$0.Children.toArray(children).find((child) => require$$0.isValidElement(child) && unwrap(child.props.when));
...
const Dynamic = ({ component: Component, ...props }) => {
    ...
    return require$$0.isValidElement(Component) ? (require$$0.cloneElement(Component, unwrappedProps)) : (jsxRuntime.jsx(Component, { ...unwrappedProps }));
};

const For = ({ each, children }) => {
    const unwrappedEach = unwrap(each);
    return (jsxRuntime.jsx(jsxRuntime.Fragment, { children: unwrappedEach.map((item, index) => (jsxRuntime.jsx(require$$0.Fragment, { children: children(item, index) }, index))) }));
};
